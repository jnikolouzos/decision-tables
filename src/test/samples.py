...
def public_domain_decision_table():
    ...
    classical_writers = Condition(rule_type=ConditionType.CONTAINS_ANY_OF, field='writers', value='Mozart,Beethoven,Bach,Strauss,Verdi,Puccini,Chopin,Wagner,Tchaikovsky,Vivaldi,Brahms,Schubert,Rachmaninoff')
    rules.append(Rule(order=2, decision=Decision(decision_type=DecisionType.BOOLEAN, value=True), conditions=[classical_writers]))

    trad_pd_dp = Condition(rule_type=ConditionType.CONTAINS_ANY_OF, field='title', value='Trad,PD,DP')
    rules.append(Rule(order=1, decision=Decision(decision_type=DecisionType.BOOLEAN, value=True), conditions=[trad_pd_dp]))

    finally_condition = Condition(rule_type=ConditionType.FINALLY)
...
def remix_decision_table_contains_any_of():
    ...
    remix = Condition(rule_type=ConditionType.CONTAINS_ANY_OF, field='title', value='mix,remix')
...
def remix_decision_table_contains():
    ...
    remix = Condition(rule_type=ConditionType.CONTAINS, field='title', value='mix')
    rules.append(Rule(order=1, decision=Decision(decision_type=DecisionType.BOOLEAN, value=True), conditions=[remix]))
